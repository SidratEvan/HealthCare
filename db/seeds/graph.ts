 ...
 *
 * ## Why this is in `db/seeds` and not in `db/tests`
 *
 ...
 * `db/tests/support/fixtures.ts` as an explicitly provisional stand-in until
 ...
 * `pnpm db:seed` uses. `db/tests/support/fixtures.ts` is a thin re-export.
 *
 ...
 * `db/tests/seeds.test.ts`, which runs `seedDemoData` and asserts the
 * `FR-DEM-*` shape.
 */
