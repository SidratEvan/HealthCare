...
//
// BACKEND.md §2:  packages/domain "has no I/O. Pure functions and types."
//
// CLAUDE.md §7:   "SQL only in repositories. Events and notifications only in
//                  services. The queue reducer exists once, in packages/domain."
//
...
//   nothing on a developer's machine while still matching in Linux CI. Zones
//   here are therefore plain directory (or file) paths — never globs. A rule
//   that is quiet on one platform is worse than no rule at all.
...
const PACKAGE_ZONES = [
  {
    ...
    from: ['apps', 'packages/client', 'packages/ui', 'packages/i18n', 'db'],
    message:
      'packages/domain has no I/O and no dependants (BACKEND.md §2). It is imported by everything and imports nothing — that is what makes replay deterministic (FR-QUE-05).',
  },
  {
    target: 'packages/ui',
    from: ['apps', 'packages/client'],
    message:
      'packages/ui is the design system. It may not import an app or the API client (FRONTEND.md §10).',
  },
  {
    target: 'packages/i18n',
    from: ['apps', 'packages/client', 'packages/ui'],
    message: 'packages/i18n holds messages and formatters only (FRONTEND.md §10).',
  },
  {
    target: 'packages/client',
    from: ['apps'],
    message: 'packages/client is consumed by apps, never the reverse (FRONTEND.md §10).',
  },
];
...
export const layering = [
    ...
    rules: {
      ...
      'no-restricted-syntax': [
        ...
        },
        {
          ...
          message:
            'The queue reducer exists once, in packages/domain (CLAUDE.md §7). Import it; never reimplement it in a route, a service or the client.',
        },
      ],
    },
  },
  ...
  // ---------------------------------------------------------------------------
  {
    files: ['packages/{client,ui}/src/**/*.{ts,tsx}'],
    rules: {
      'no-restricted-syntax': [
        'error',
        {
          selector: 'FunctionDeclaration[id.name=/^(queueReducer|reduceQueue)$/]',
          message:
            'The queue reducer exists once, in packages/domain (CLAUDE.md §7). Import it; never reimplement it in the client.',
        },
      ],
    },
  },
  ...
  // ---------------------------------------------------------------------------
  // packages/domain: pure, deterministic, replayable
  //
          ...
          patterns: [
            {
              ...
              message:
                'packages/domain has no I/O (BACKEND.md §2). Pass the value in as an argument.',
            },
            {
              group: ['kysely', 'pg', 'express', 'socket.io*', 'pg-boss', 'pino', 'react', 'next*'],
              message:
                'packages/domain is pure types and functions, shared by the server and the client (BACKEND.md §2).',
            },
          ],
        },
      ],
  ...
  {
    files: ['packages/domain/src/queue/**/*.ts', 'packages/domain/src/emergency/**/*.ts'],
    rules: {
      'no-restricted-syntax': [
        'error',
        {
      ...
      'no-restricted-globals': [
        'error',
        { name: 'fetch', message: 'packages/domain has no I/O (BACKEND.md §2).' },
        { name: 'process', message: 'packages/domain reads no environment (BACKEND.md §2).' },
      ],
    },
  },
];
